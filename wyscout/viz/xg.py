...
def plot_xg_line_chart(
    team_id: int,
    evnts: List[Dict[str, any]],
    colours: List[str],
    match_details,
    team_details,
    ax,
    ...
    ax.legend(
        custom_lines, [team_details[int(t)]["name"] for t in match_details["teamsData"]]
    )


def plot_attacking_stats(
    team_id: int,
    season_id: int,
    colors: List[str] = ["royalblue", "yellow"],
    edgecolors: list[str] = ["royalblue", "royalblue"],
):
    sorted_players, team_details = _get_attacking_stats_data(team_id, season_id)
    ...
    # Chart 1
    ax1.bar(
        [p[0] for p in sorted_players],
        [p[1]["xgDiff"] for p in sorted_players],
        color=colors[0],
        label="Post-shot xG minus xG",
        zorder=3,
    )
    ...
    ax2.bar(
        x - width / 2,
        [p[1]["xg"] for p in sorted_players],
        width=width,
        label="xG",
        color=colors[0],
    ...
    ax2.bar(
        x + width / 2,
        [p[1]["goals"] for p in sorted_players],
        width=width,
        label="Goals",
        color=colors[1],
    ...
    ax3.bar(
        x - width / 2,
        [p[1]["attempts"] for p in sorted_players],
        width=width,
        label="Attempts",
        color=colors[0],
        edgecolor=edgecolors[0],
    ...
    ax3.bar(
        x + width / 2,
        [p[1]["onTarget"] for p in sorted_players],
        width=width,
        label="On target",
        color=colors[1],
        edgecolor=edgecolors[1],
    ...
    ax4.bar(
        [p[0] for p in sorted_players],
        [p[1]["avexg"] for p in sorted_players],
        color=colors[0],
        label="Average xG per attempt",
        zorder=3,
    )
...
def _get_attacking_stats_data(team_id: int, season_id: int):
    ...
    players = defaultdict(
        lambda: {"xg": 0, "postShotXg": 0, "attempts": 0, "onTarget": 0, "goals": 0}
    )
    ...
    for m in matches:
        for e in m["events"]:
            if e["type"]["primary"] == "shot":
                ...
                players[e["player"]["name"]]["postShotXg"] += (
                    e["shot"]["postShotXg"]
                    if e["shot"]["postShotXg"] is not None
                    else 0
                )
                players[e["player"]["name"]]["attempts"] += 1
                players[e["player"]["name"]]["onTarget"] += (
                    1 if e["shot"]["postShotXg"] is not None else 0
                )
                players[e["player"]["name"]]["goals"] += (
                    1 if e["shot"]["isGoal"] == True else 0
                )

    for p in players:
        players[p]["xgDiff"] = players[p]["postShotXg"] - players[p]["xg"]
        players[p]["onTargetPct"] = players[p]["onTarget"] / players[p]["attempts"]
        players[p]["avexg"] = players[p]["xg"] / players[p]["attempts"]

    sorted_players = sorted(players.items(), key=lambda x: x[1]["xgDiff"], reverse=True)
